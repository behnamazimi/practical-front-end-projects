...
class ElementsHelper {
    ...
    assignElements() {
        // Quiz Card Elements
        this.quizCard.startBtn = this.quizCard.querySelector(".quiz-details__start-btn");
        this.quizCard.titleElm = this.quizCard.querySelector(".quiz-details__title");
        this.quizCard.descriptionElm = this.quizCard.querySelector(".quiz-details__description");
        this.quizCard.metaQCElm = this.quizCard.querySelector(".quiz-details__meta.--qc strong");
        this.quizCard.metaTimeElm = this.quizCard.querySelector(".quiz-details__meta.--t strong");

        // Question Card Elements
        this.questionCard.progressRemainingTimeElm = document.querySelector(".questions-card__remaining-time");
        this.questionCard.progressQuestionCountElm = document.querySelector(".questions-card__q-count");
        this.questionCard.progressbarElm = document.querySelector(".questions-card__progress .--value");
        this.questionCard.questionTitleElm = document.getElementById("question-title");
        this.questionCard.optionOneElm = document.querySelector("#option-one ~ label");
        this.questionCard.optionTwoElm = document.querySelector("#option-two ~ label");
        this.questionCard.optionThreeElm = document.querySelector("#option-three ~ label");
        this.questionCard.optionFourElm = document.querySelector("#option-four ~ label");
     ...
     */
    initListeners() {
        this.quizCard.startBtn.addEventListener("click", this.showQuestionsCard.bind(this));
        this.questionCard.nextBtn.addEventListener("click", this.nextBtnHandler.bind(this));
        this.questionCard.stopBtn.addEventListener("click", this.stopBtnHandler.bind(this));
        this.resultCard.gotoHome.addEventListener("click", this.hideResultCard.bind(this));
    }
    ...
    _setProgressTicker() {
        this.remainingTimeInterval = setInterval(() => {
            ...
            if (qTime && qTime.remainingTime) {
                ...
                this.questionCard.progressRemainingTimeElm.innerText = qTime.remainingTime;
    ...
    parseNextQuestion(question) {
        ...
        this.questionCard.progressQuestionCountElm.innerText = `Question ${this.quiz._currentQuestionIndex + 1}/${this.quiz._questions.length}`;
        this.questionCard.questionTitleElm.setAttribute("data-qn", `Q ${this.quiz._currentQuestionIndex + 1}:`);
